...
function loadStoredBowlers() {
  try {
    const raw = window.localStorage.getItem(BOWLERS_KEY);
    if (!raw) return [];
    ...
    return parsed
      .map(bowler => {
        if (typeof bowler === "string") return bowler.trim();
...
export default function TeamManagement({
  ...
  const availableBowlers = Array.from(
    new Set([
        ...
        <div key={team.id} style={S.card}>
          ...
          >
            ...
            {availableBowlers.length > 0 ? (
              ...
              >
                No bowlers available. Add bowlers first.
              </div>
            )}
          </div>
        </div>
